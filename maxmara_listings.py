...
image_hashes_path = '/Users/Ziyu/OneDrive - Clarivate Analytics/Desktop/weekendmaxmara_images/hashes.csv'
maxmara_images_directory_path = '/Users/Ziyu/OneDrive - Clarivate Analytics/Desktop/weekendmaxmara_images/'
...
for i in range(64):
    os.makedirs('/Users/Ziyu/OneDrive - Clarivate Analytics/Desktop/maxmara/comparison/%d/' % (i))

for i in range(len(listings)):
    try:
        image_path = download_image(listings.iloc[i]['Thumbnail URL'])
        index = listings.iloc[i]['Item ID (Required|Non-editable)']
        image = Image.open(image_path)
        ...
        for closest_image_name in closest_image_names:
            v_merge([maxmara_images_directory_path + closest_image_name, image_path], comparison_directory_path + '%d/%s-%s' % (minimum_distance, str(index), closest_image_name))
